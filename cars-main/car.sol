...
contract D2dNft is ERC721, ReentrancyGuard, Ownable {
    using Counters for Counters.Counter;
    mapping(address => uint256) private _balances;

    constructor(string memory _customBaseURI, string memory _ownerBaseURI) ERC721("testdt", "tdt") {
       // customBaseURI = "https://gateway.pinata.cloud/ipfs/QmWNyYuD3M9KdY86E8CHCBM8SPcpacG99AHWt9erEwDozm/";
        customBaseURI = _customBaseURI;
        ownerBaseURI = _ownerBaseURI;
    ...
    function addPacks() private onlyOwner {
        packs[id] = Pack("Newaie", "0.0361746", 1500, 1500, 0);
        id++;
        packs[id] = Pack("Entry", "0.05", 1500, 1500, 0);
        id++;
        packs[id] = Pack("Basic", "0.1", 1000, 1000, 300);
        id++;
        packs[id] = Pack("Standard", "0.15", 1000, 1000, 450);
        id++;
        packs[id] = Pack("Premium", "0.2", 1000, 1000, 600);
        id++;
        packs[id] = Pack("Supreme", "0.25", 100, 100, 750);
        id++;
        packs[id] = Pack("Challenger", "0.5", 50, 50, 1500);
        id++;
        packs[id] = Pack("Profesional", "1", 50, 50, 2700);
    }

    /** PackDetails **/

    function PackName(uint256 _id) public view returns (string memory) {
        return packs[_id].Name;
    }

    function PackPrice(uint256 _id) public view returns (string memory) {
        return string(abi.encodePacked(packs[_id].Price, " ", "WETH"));
    }

    function PackSupply(uint256 _id) public view returns (uint256) {
        return packs[_id].TotalSupply;
    }

    function PackLeft(uint256 _id) public view returns (uint256) {
        return packs[_id].Left;
    }

    /** MINTING **/

    uint256 public constant MAX_SUPPLY = 6200;

    Counters.Counter private supplyCounter;
    string private ext = ".json";

    function mint(uint256 pack_id)
        public
        nonReentrant
        onlyOwner
    {
        require(saleIsActive, "Sale not active");
        require( packs[pack_id].Left != 0, "no mints left");
   ...
   /*
        ...
        onlyOwner
    {
        ...
        require( packs[pack_id].Left != 0, "no mints left");
        supplyCounter.increment();
        uint num = packs[pack_id].TotalSupply - packs[pack_id].Left
        _mint(msg.sender, supplyCounter.current());
        string memory uri_ = string(
            abi.encodePacked(
                customBaseURI,
                Strings.toString(pack_id),
   ...
   */ 
    ...
    function BuyByERC20(
        ...
        require(_amount != 0);
        require(packs[_id].Ftoken == _amount);
    ...
    function _claimWinner(address tokenAddr, address from, address admin, uint256 amount, uint256 Id, address conaddress) internal{
