...
contract Garage is ERC721,  Ownable {
    using Counters for Counters.Counter;
    constructor(address __owner) ERC721("Parts", "PT") {
        admin = __owner;
        addWorkshops();
        addUris();
    }
    ...
    uint256 public Type1 = 0;
    uint256 public Type2 = 0;
    uint256 public Type3 = 0;
    uint256 public Type4 = 0;
    uint256 public Type5 = 0;
    uint256 public Type6 = 0;

    Counters.Counter private supplyCounter;
    string private ext = ".json";
    mapping(address => uint256) private _balances;
    uint256 constant Max_supply = 3200;
    ...
    mapping(uint256 => URI) public uriS;
    ...
    function addWorkshops() private {
        uint256 ID = 1;

        workshops[ID] = Workshop(" Common", "0.5", 5 minutes, 1);
        ID++;
        workshops[ID] = Workshop(" Uncommon", "1", 5 minutes, 2);
        ID++;
        workshops[ID] = Workshop(" private Type", "2.5", 3 minutes, 1);
        ID++;
        workshops[ID] = Workshop(" Unique", "5", 3 minutes, 2);
        ID++;
        workshops[ID] = Workshop(" Epic", "10", 1 minutes, 1);
        ID++;
        workshops[ID] = Workshop(" Mythic", "15",1 minutes, 2);
        ID++;
    }

    function addUris() private {
        uint256 i = 1;
        uriS[i] = URI("https://gateway.pinata.cloud/ipfs/QmcAkbPgJ9sM89JfNAVRvgCFUxaxqdNv7vqpX9yu5HdWcP/");
        i++;
        uriS[i] = URI("https://gateway.pinata.cloud/ipfs/QmV7jDAFq5boyqsHpCuitiQwdFGe6xKiw18JMAk2kNw6fq/");
        i++;
        uriS[i] = URI("https://gateway.pinata.cloud/ipfs/QmWjXJebskB2yhUDet7PB6bL4RSEgY7M8YTGrVbVradVVs/");
        i++;
        uriS[i] = URI("https://gateway.pinata.cloud/ipfs/QmPRMU6Br8BLbaPFQNnnXcxjpuRkrxP3PQXtqpXKH7S2Sp/");
        i++;
        uriS[i] = URI("https://gateway.pinata.cloud/ipfs/QmZ1au3XxhJpXyNmFJXAMCqTVwuBKVkpp8Uo2Lm3GrDsZi/");
        i++;
        uriS[i] = URI("https://gateway.pinata.cloud/ipfs/QmeQrQ1niDx3YMFpPPGHDmWcFDR4dePJZqBzMtR9qq3dzr/");
        i++;
    }

    // workshop details 

    function workshopPrice(uint256 _ID) public view returns (string memory) {
        return string(abi.encodePacked(workshops[_ID].price, " ", "WETH"));
    }

    function workshopType(uint256 _ID) public view returns (string memory) {
        return workshops[_ID].name;
    }

    function workshopDuration(uint256 _ID) public view returns (uint256) {
        return workshops[_ID].duration;
    }

    function workshopParts(uint256 _ID) public view returns (uint256) {
        return workshops[_ID].parts;
    }

    // Supply and time tracker
    ...
    function production(uint256 _ID, string memory _val) public returns (bool) {
        require(supplyCounter.current() <= Max_supply, "no mints left");
        string memory data = workshops[_ID].price;
        require(
            keccak256(abi.encodePacked(_val)) ==
                keccak256(abi.encodePacked(data)),
            "incorrect amount"
        );
        timer[msg.sender][_ID] = block.timestamp;
        return true;
    }

    function mint(uint256 _ID) public {
        uint256 _now = block.timestamp;
        uint256 _end = timer[msg.sender][_ID] + workshops[_ID].duration;
        uint256 _count = workshops[_ID].parts;
        require(_now >= _end, "Duration is not satisfied ");

        for (uint256 i = 0; i < _count; i++) {
            done();
        }
        timer[msg.sender][_ID] = block.timestamp; // resetting timer.
    }
    ...
    function TokenUri_ (uint256 _tokenId) public view returns (string memory){
        return tokenURIMap[_tokenId];
    }

    function RandomParts(uint256 _modulus) internal view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function done() internal {
        uint256 jj = 10 * RandomParts(100);
        
        // 50% common 
        if (jj <= 499) {
            if (Type1 <= 1601) {
                uint256 k = RandomParts(2);
                string memory data = uriS[1].urii;
                Type1++;
                supplyCounter.increment();
                _mint(admin, supplyCounter.current());
                string memory uri_ = string(
                    abi.encodePacked(
                        data,
                        Strings.toString(1),
                        Strings.toString(k),
                        Strings.toString(supplyCounter.current()),
                        ext
                    )
                );
                
                setTokenURI(supplyCounter.current(), uri_);
                //transferFrom(admin, msg.sender, supplyCounter.current());
            } else {
                done();
            }
        }
        ...
        if (jj > 499 && jj <= 749) {
            if (Type2 <= 800) {
                uint256 k = RandomParts(2);
                string memory data = uriS[2].urii;
                Type2++;
                supplyCounter.increment();
                _mint(admin, supplyCounter.current());
                string memory uri_ = string(
        ...
        if (jj > 749 && jj <= 899) {
            if (Type3 <= 480) {
                uint256 k = RandomParts(2);
                string memory data = uriS[3].urii;
                Type3++;
                supplyCounter.increment();
                _mint(admin, supplyCounter.current());
                string memory uri_ = string(
        ...
        if (jj > 899 && jj <= 989) {
            if (Type4 <= 288) {
                uint256 k = RandomParts(2);
                string memory data = uriS[4].urii;
                Type4++;
                supplyCounter.increment();
                _mint(admin, supplyCounter.current());
                string memory uri_ = string(
        ...
        if (jj > 989 && jj <= 998) {
            if (Type5 <= 50) {
                uint256 k = RandomParts(2);
                string memory data = uriS[5].urii;
                Type5++;
                supplyCounter.increment();
                _mint(admin, supplyCounter.current());
                string memory uri_ = string(
        ...
        if (jj == 999) {
            if (Type6 <= 3) {
                uint256 k = RandomParts(2);
                string memory data = uriS[6].urii;
                Type6++;
                supplyCounter.increment();
                _mint(admin, supplyCounter.current());
                string memory uri_ = string(
